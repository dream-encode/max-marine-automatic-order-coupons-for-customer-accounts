 ...
 * @since      1.0.0
 *
 * @package    Max_Marine_Automatic_Order_Coupons_For_Customer_Accounts/includes/abstracts
 */

namespace Max_Marine\Automatic_Order_Coupons_For_Customer_Accounts\Core\Abstracts;
 ...
 *
 * @package  Max_Marine\Automatic_Order_Coupons_For_Customer_Accounts\Core\Abstracts\Max_Marine_Automatic_Order_Coupons_For_Customer_Accounts_Abstract_WC_Logger
 * @author   David Baumwald <david@dream-encode.com>
 */
class Max_Marine_Automatic_Order_Coupons_For_Customer_Accounts_Abstract_WC_Logger {
	...
	public static function log( $data, $level = 'debug' ) {
		...
		$log_level_setting = max_marine_automatic_order_coupons_for_customer_accounts_get_plugin_setting( 'log-level', 'off' );
		...
		if ( ! function_exists( 'wc_get_logger' ) ) {
			...
			error_log(
				sprintf(
					...
					__( '%1$s->%2$s depends on `wc_get_logger` which is bundled with the WooCommerce plugin.', 'max-marine-automatic-order-coupons-for-customer-accounts' ),
					__CLASS__,
					__METHOD__
				)
			);
